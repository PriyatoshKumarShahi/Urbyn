          ...
          <div key={item._id || index} className="rounded-[18px] border-[3px] border-ink bg-white p-3 shadow-brutalSm">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <div className="text-sm text-slate-600">{item.areaName || 'Unknown area'} • {item.category}</div>
                <div className="text-xs font-semibold text-slate-500">Reported by {item.createdBy}</div>
              </div>
              <div className="rounded-full border-[3px] border-ink bg-butter px-3 py-1 text-sm font-black">{item.hotspotWeight}</div>
            </div>
          </div>
        ))}
      </div>
